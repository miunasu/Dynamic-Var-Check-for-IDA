...
class DVC:
    ...
    def get_name(self, point):
        ...
        while count <= 3:
            ...
            if symbol_name != '' and any((i in symbol_name for i in api_key_word)):
...
def register_actions():
    ...
    idaapi.attach_action_to_menu("Debugger/DVC/", "block var check", idaapi.SETMENU_APP)
    idaapi.attach_action_to_menu("Debugger/DVC/", "func var check", idaapi.SETMENU_APP)
    idaapi.attach_action_to_menu("Debugger/DVC/", "sub func disassemble", idaapi.SETMENU_APP)

def unregister_actions():
    idaapi.unregister_action("block var check") 
    idaapi.unregister_action("func var check")
    idaapi.unregister_action("sub func disassemble")

class DVC_PLUGIN(idaapi.plugin_t):
    flags = idaapi.PLUGIN_KEEP
    comment = "DVC_PLUGIN"
    help = "DVC_PLUGIN"
    wanted_name = "Dynamic Var Check"
